...
// reconciler/reconciler.js
var import_react = __toESM(require_react(), 1);
var import_react_reconciler = __toESM(require_react_reconciler(), 1);

// reconciler/tree-node.js
var nextNodeId = 1;
...
var hostConfig = {
  ...
  insertBefore(parent, child, beforeChild) {
    console.debug(`insertBefore: ${parent.type} <- ${child.type || `"${child.text}"`} before ${beforeChild.type || `"${beforeChild.text}"`}`);
    const index = parent.children.indexOf(beforeChild);
    if (index === -1) {
      console.error(`insertBefore: beforeChild not found in parent! Appending instead.`, {
        parent: parent.type,
  ...
  insertInContainerBefore(container, child, beforeChild) {
    ...
    if (index !== -1) {
      ...
      console.error(`insertInContainerBefore: beforeChild not found in rootChildren! Appending instead.`, { beforeChild: beforeChild.type });
      container.rootChildren.push(child);
    }
    child.parent = null;
  },
...
var host_config_default = hostConfig;

// reconciler/reconciler.js
var reconciler = import_react_reconciler.default(host_config_default);
function createRoot() {
  const container = {
    rootChildren: []
  };
  const fiberRoot = reconciler.createContainer(container, 0, null, false, null, "", (error, errorInfo) => {
