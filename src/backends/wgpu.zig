...
const default_msdf_params = MsdfParams{
...
msdf_shader_module: ?*wgpu.ShaderModule = null,
msdf_bind_group_layout: ?*wgpu.BindGroupLayout = null,
msdf_pipeline_layout: ?*wgpu.PipelineLayout = null,
msdf_pipeline: ?*wgpu.RenderPipeline = null,

globals_buffer: ?*wgpu.Buffer = null,
globals_bind_group: ?*wgpu.BindGroup = null,
msdf_buffer: ?*wgpu.Buffer = null,
...
pub fn deinit(self: *WgpuBackend) void {
    ...
    if (self.msdf_pipeline) |pipeline| {
        pipeline.release();
        self.msdf_pipeline = null;
    }
    if (self.pipeline_layout) |layout| {
        layout.release();
        self.pipeline_layout = null;
    }
    if (self.msdf_pipeline_layout) |layout| {
        layout.release();
        self.msdf_pipeline_layout = null;
    }
    if (self.shader_module) |module| {
        module.release();
        self.shader_module = null;
    }
    if (self.msdf_shader_module) |module| {
        module.release();
        self.msdf_shader_module = null;
    }
    if (self.texture_bind_group_layout) |layout| {
        layout.release();
        self.texture_bind_group_layout = null;
    }
    if (self.msdf_bind_group_layout) |layout| {
        layout.release();
        self.msdf_bind_group_layout = null;
    }
...
pub fn drawClippedTriangles(
    ...
    self.draw_commands.appendAssumeCapacity(.{
        ...
        .msdf_params = if (resource.kind == .msdf and self.pending_msdf_params_valid) self.pending_msdf_params else default_msdf_params,
    });

    if (resource.kind == .msdf) {
        self.pending_msdf_params_valid = false;
    }
}

pub fn setMsdfParams(self: *WgpuBackend, fill_color: dvui.Color, outline_color: dvui.Color, px_range: f32, outline_width_px: f32) void {
    self.pending_msdf_params = .{
        .fill_color = colorToFloat4(fill_color),
        .outline_color = colorToFloat4(outline_color),
        .px_range = px_range,
        .outline_width_px = outline_width_px,
    };
    self.pending_msdf_params_valid = true;
}
...
pub fn textureCreateMsdf(
    ...
    const texture = try self.createTextureResourceKind(width, height, interpolation, pixels, .msdf);
    return dvui.Texture{ .ptr = texture, .width = width, .height = height };
}
...
fn ensurePipeline(self: *WgpuBackend) !void {
    ...
    if (self.shader_module == null) {
        const shader = self.device.createShaderModule(&wgpu.shaderModuleWGSLDescriptor(.{ .code = raster_shader_source })) orelse return dvui.Backend.GenericError.BackendError;
...
fn ensureMsdfPipeline(self: *WgpuBackend) !void {
    ...
    if (self.msdf_bind_group_layout == null) {
        ...
        self.msdf_bind_group_layout = layout;
    }

    if (self.msdf_pipeline_layout == null) {
        const layout = self.device.createPipelineLayout(&wgpu.PipelineLayoutDescriptor{
            .label = wgpu.StringView.fromSlice("dvui msdf pipeline layout"),
            .bind_group_layout_count = 3,
            .bind_group_layouts = &[_]*wgpu.BindGroupLayout{
                self.globals_bind_group_layout.?,
                self.texture_bind_group_layout.?,
                self.msdf_bind_group_layout.?,
            },
        }) orelse return dvui.Backend.GenericError.BackendError;
        self.msdf_pipeline_layout = layout;
    }

    if (self.msdf_shader_module == null) {
        ...
        self.msdf_shader_module = shader;
    }
...
fn createTextureResource(
    ...
    return self.createTextureResourceKind(width, height, interpolation, pixels, .raster);
}
...
fn colorToFloat4(color: dvui.Color) [4]f32 {
    return .{
...
const shader_source = msdf_zig.shader.ui_wgsl;

const raster_shader_source =
    \\struct Globals {
