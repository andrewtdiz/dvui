...
pub const RectScale = struct {
    ...
    pub fn rectToRectScale(self: *const RectScale, r: Rect) RectScale {
        return .{ .r = r.scale(self.s, Rect.Physical).offset(self.r), .s = self.s };
    }

    pub fn rectToPhysical(self: *const RectScale, r: Rect) Rect.Physical {
        return r.scale(self.s, Rect.Physical).offset(self.r);
    }

    pub fn rectFromPhysical(self: *const RectScale, r: Rect.Physical) Rect {
        return r.offsetNeg(self.r).scale(1 / self.s, Rect);
    }

    pub fn pointToPhysical(self: *const RectScale, p: Point) Point.Physical {
        return p.scale(self.s, Point.Physical).plus(self.r.topLeft());
    }

    pub fn pointFromPhysical(self: *const RectScale, p: Point.Physical) Point {
        return p.diff(self.r.topLeft()).scale(1 / self.s, Point);
    }
